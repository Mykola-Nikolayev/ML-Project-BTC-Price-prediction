...
try:
    ...
    # Définir le plancher à zéro
    data_prophet['floor'] = 0

    # Initialiser le modèle Prophet avec un plancher
    print("Initialisation du modèle Prophet avec un plancher...")
    ...
    # Ajuster le modèle
    print("Entraînement du modèle Prophet avec le plancher...")
    ...
    future['floor'] = 0  # Ajouter le plancher
    ...
    phases_df['start_price'] = phases_df['start_date'].apply(
        lambda x: data.loc[data['date'] == x, 'priceClose'].values[0])
    phases_df['end_price'] = phases_df['end_date'].apply(
        lambda x: data.loc[data['date'] == x, 'priceClose'].values[0])
    ...
    def simulate_price_with_historical_cycles(last_price, total_days, avg_bull_duration, avg_bull_return,
                                              avg_bear_duration, avg_bear_return, volatility):
        """
    ...
    fig.update_layout(
        ...
        legend=dict(x=0, y=1),
        yaxis=dict(range=[0, forecast['yhat_upper'].max() * 1.1])  # Commencer l'axe des y à zéro
    )
